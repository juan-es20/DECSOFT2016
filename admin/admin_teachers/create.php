<?php 
	...
	if ( !empty($_POST)) {
		...
		if (empty($pass)) {
			$passError = 'Por favor ingrese un password';
			$valid = false;
		}
		...
		if ($valid) {
			...
			$sql = "INSERT INTO teachers (teacher_name,teacher_last_name,teacher_email,teacher_pass) values(?, ?, ?, ?)";
			$q = $pdo->prepare($sql);
			$q->execute(array($name,$last_name,$email,md5($pass)));
			Database::disconnect();
